...
- NTFS 增量索引缓存保存到 `%LOCALAPPDATA%\DiskCompare\IndexCache`，只用于加速当前盘后续快照。
- 快照会保存文件夹大小聚合；对比时优先使用聚合数据生成变化树，旧快照没有聚合时自动回退逐文件聚合。
- 遇到无权限、路径过长、重解析点等目录时跳过并记录错误计数，避免单个目录导致全盘扫描失败。

## 快照逻辑

当前实现采用类似 Everything 的元数据思路：NTFS 本地盘优先只读打开 `\\.\X:` 并解析 MFT 记录，直接从文件系统元数据中构建路径和大小；完成全量 MFT 快照后，会保存一份应用私有索引缓存。后续同一卷的快照如果发现卷序列号、USN Journal ID 和 USN 范围都匹配，则只读取 `FSCTL_QUERY_USN_JOURNAL` 与 `FSCTL_READ_USN_JOURNAL` 返回的增量记录，并按变更记录号重新读取对应 MFT 记录。
